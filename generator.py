...
def build_extra_pool(category_rewards, normal_queue_counts):
    ...
    for category, rewards in category_rewards.items():
        ...
        card_used = queue_count * card["limit_per_queue"]

        for index in range(card_used, card["stock"]):
        ...
        s_used = queue_count * feather_s["limit_per_queue"]

        for index in range(s_used, feather_s["stock"]):
            page, slot = position(index)
        ...
        a_used = queue_count * feather_a["limit_per_queue"]

        # A เริ่มหลัง S stock ของ category นั้น
        a_start = feather_s["stock"] + a_used

        a_end = feather_s["stock"] + feather_a["stock"]
...
def generate_extra_queues(guild_id, extra_pool, limits, start_no):
    ...
    extra_queue_count = min(
        len(extra_pool["CARD"]) // limits["CARD"],
        len(extra_pool["FEATHER_S"]) // limits["FEATHER_S"],
        len(extra_pool["FEATHER_A"]) // limits["FEATHER_A"],
    )
    ...
    remaining = {
        reward: (len(extra_pool[reward]) - pointers[reward]) for reward in REWARD_TYPES
    }
